...
class Pixel {
  ...
  draw() {
    ...
    this.ctx.globalAlpha = Math.max(0, Math.min(1, this.alpha))
  ...
  appear() {
    ...
    // Smooth fade-in for a more premium feel
  ...
  disappear() {
    ...
    // Premium ease-out dissolve: ease size/alpha toward 0 (no abrupt steps)
    this.size += (0 - this.size) * 0.03
    this.alpha *= 0.985

    if (this.size <= 0.02 || this.alpha <= 0.02) {
      this.isIdle = true
      return
    }
...
class PixelCanvasElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this._parent?.addEventListener("mouseenter", this.onMouseEnter)
    this._parent?.addEventListener("mouseleave", this.onMouseLeave)

    if (!this.noFocus) {
      this._parent?.addEventListener("focus", this.onFocus, { capture: true })
      this._parent?.addEventListener("blur", this.onBlur, { capture: true })
    }
  }
  ...
  disconnectedCallback() {
    ...
    this._parent?.removeEventListener("mouseenter", this.onMouseEnter)
    this._parent?.removeEventListener("mouseleave", this.onMouseLeave)

    if (!this.noFocus) {
      this._parent?.removeEventListener("focus", this.onFocus, { capture: true })
      this._parent?.removeEventListener("blur", this.onBlur, { capture: true })
    }
